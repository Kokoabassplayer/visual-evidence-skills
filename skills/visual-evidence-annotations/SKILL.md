...
When command execution is available, prefer the bundled helper for deterministic file annotation:

```bash
node scripts/visual-evidence.mjs annotate screenshot.svg --box "120,80,360,120" --label "Check here"
node scripts/visual-evidence.mjs annotate screenshot.png --circle "240,160,48" --arrow "160,120,220,150" --label "Wrong value"
```

The helper writes an annotated SVG by default. Use browser overlays or harness-native screenshot tools when marking a live page is more accurate than annotating an existing image file.
